...
| 分类 | 选项 |
|------|------|
| **📌 基础设置** | 自动扫描、默认图片文件夹、包含子文件夹 |
| **🏠 主页布局** | 每行图片数、卡片间距、卡片圆角、固定图片高度、默认排序/筛选 |
| **🖼️ 图片卡片** | 纯净画廊模式、自适应大小、统一卡片高度、显示名称/大小/尺寸/序号/锁定图标、名称换行、悬停效果 |
| **🗑️ 删除与回收站** | 删除前确认、移到系统回收站、启用插件回收站、恢复路径 |
| **🔗 引用与预览** | 前往笔记时保持详情页、显示引用时间、鼠标滚轮默认模式 |
| **🔄 重命名设置** | 自动生成文件名、路径命名深度、重名处理方式、多笔记引用处理、保存批量重命名日志 |
| **⚡ 性能优化** | 启用懒加载、懒加载延迟、最大缓存数量、增量扫描缓存 |
| **🔍 搜索设置** | 区分大小写、实时搜索延迟、搜索包含路径 |
| **📦 批量操作** | 最大批量操作数（默认100）、批量确认阈值、显示批量进度 |
| **🔒 锁定文件** | 锁定列表管理、显示文件位置、批量解锁 |
| **📊 统计信息** | 显示统计信息、统计位置（顶部/底部） |
| **📋 操作日志** | 日志级别、输出到控制台、启用调试日志、查看/清除日志 |
| **⌨️ 键盘快捷键** | 自定义所有快捷键、重置为默认值 |
| **🔄 MD5 去重** | 启用去重功能、哈希缓存管理 |
