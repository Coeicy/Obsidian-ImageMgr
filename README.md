...
| 🈳 **空链接检测** | 双标签页设计（本地链接/网络链接），支持🔄重新缓存、一键跳转、搜索筛选 |
...
### v1.0.3 (2026-02-01) - **智能黑名单与错误处理系统**

#### 🚀 新增功能
- ✅ **智能黑名单管理系统** - 容量无限，自动检测失效域名并屏蔽
  - 智能内存管理（活跃记录 + 自动归档）
  - 连续失败2次自动加入黑名单
  - 失败率60%触发自动屏蔽
  - 本地持久化缓存（7天自动过期）
...
- ✅ **网络图片错误处理系统** - 智能分类与处理
  - 9种错误类型分类（网络错误、超时、验证错误等）
  - 缓存命中率60-80%
  - 重试成功率40-60%
  - 错误提示减少80%+
  
- ✅ **空链接页面增强** - 黑名单可视化
  - 页面顶部显示黑名单内容（URL、错误、时间）
  - 内存缓存优化，加载速度提升10-100倍
  - 修复空分类标签显示问题
  
- ✅ **设置页面优化** - 黑名单文本框增强
  - 支持调整大小（宽度100%，高度200px）
  - 等宽字体显示，提升URL可读性
  - 批量重命名日志默认开启
...
- **云端图片缓存**（四类数据，仅元数据、不存图片二进制）：
  - **网络图片记录**：每张网络图的 URL、来源文件、行号、验证状态等，主键为「文件名+URL」组合哈希，用于增量扫描与操作记录
  - **已扫描文件索引**：每个 Markdown 的路径、修改时间、内容哈希、所含网络图 ID 列表，用于只重扫有改动的文件
  - **系统元数据**：最后扫描时间、图片总数、缓存命中率等统计
  - **失效 URL 黑名单**：验证失败的链接及原因，避免重复请求，并在空链接页展示
...
#### 空链接检测
- **双标签页设计**：本地链接和网络链接分开显示，更清晰的管理体验
- **本地链接**：检测笔记中指向不存在文件的图片链接，显示文件路径、行号，支持一键跳转
- **网络链接**：检测失效的网络图片链接（404、DNS错误、连接超时等），支持🔄重新缓存
...
- 通过 MD5 哈希值检测**本地**重复图片，避免冗余存储、节省空间
- **仅针对本地图片**：云端图片为外链不占本地空间，不参与去重检测
- 哈希缓存管理，自动缓存计算结果
- 显示重复图片列表，支持批量删除（仅本地）
...
| 类型 | 算法 | 对象 | 主要作用 |
|------|------|------|----------|
| **云端图片 ID** | SHA-256 | 文件名+URL 组合 | **云端图片**：缓存主键、操作记录按图追踪、详情页显示为「URL 哈希」。仅改链接或仅改文件名时可**承接**旧记录；同时改链接和文件名则视为新图、旧记录不承接。黑名单仍按「URL 的 SHA-256」单独存储。 |
| **MD5 哈希** | MD5 | 图片文件内容 | **本地/回收站图片**：重复检测与去重、锁定列表三要素之一、操作日志按图片追踪；详情页显示为「MD5 哈希」。 |

- **云端图片**：稳定 ID = SHA-256(文件名 + "|" + URL)。只改链接或只改文件名时，新记录会承接旧记录（同一 ID），操作历史保留；**同时改链接和文件名**才会导致旧哈希记录丢失。
- **本地图片**：用 MD5 标识「哪份文件内容」，便于去重、锁定与历史追踪。
...
- **空链接检测**：阅读[空链接检测功能指南](./BROKEN_LINKS_GUIDE.md)了解详细使用方法
- **开发者**：参考[API文档](./API_DOCUMENTATION.md)、[技术指南](./TECHNICAL_GUIDE.md)和[使用示例](./EXAMPLES.md)
- **贡献者**：阅读[技术指南](./TECHNICAL_GUIDE.md)和[更新日志](./CHANGELOG.md)
...
#### 去重功能
- ✅ **MD5 去重** - 通过 MD5 哈希检测本地重复图片（云端图片不参与）
- ✅ **重复检测** - 显示本地重复图片列表，支持批量删除
- ✅ **哈希缓存** - 自动缓存 MD5 计算结果
...
- ✅ **缩放功能** - 放大/缩小预览
- ✅ **旋转功能** - 旋转图片预览
- ✅ **信息显示** - 显示图片详细信息；**哈希**：云端图片显示稳定 ID（文件名+URL 组合的 SHA-256），本地/回收站显示 MD5
- ✅ **编辑功能** - 修改文件名和路径
- ✅ **引用信息** - 显示图片的引用情况
- ✅ **操作记录** - 显示图片的操作历史
...
### 🔄 MD5 去重
- **启用去重功能**：启用后扫描时计算并检测**本地**重复图片（云端图片不参与去重）
- **哈希缓存管理**：管理 MD5 哈希缓存

### ☁️ 网络图片
- **扫描网络图片**：自动扫描笔记中的网络图片链接并写入缓存
- **清除缓存**：设置页「网络图片」中可一键清空缓存（图片记录、文件索引、黑名单等），下次扫描将重新建立
