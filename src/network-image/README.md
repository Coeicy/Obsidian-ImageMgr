...
网络图片缓存与增量扫描系统是一个高性能的文档扫描解决方案，通过 IndexedDB 本地缓存和智能增量算法，实现 10-20 倍的性能提升。

## 缓存数据说明

### 存储位置

缓存存储在**浏览器的 IndexedDB** 中，同时在插件文件夹下保留 JSON 备份：
...
**重要说明**：
- **IndexedDB**：主要存储位置，提供高性能的键值查询
- **JSON 备份**：便于手动查看、备份和迁移（随仓库一起复制即可保留缓存）
- **仅存储元数据**：不存储图片二进制数据，图片仍从原始 URL 加载

### 缓存包含的内容

缓存由 4 个 ObjectStore（表）组成：

| ObjectStore | 对应文件 | 说明 |
|-------------|----------|------|
| **images** | images.json | 网络图片记录：主键为 **文件名+URL 组合** 的 SHA-256 哈希（稳定 ID）；含 name、url、urlHash、nameHash、来源路径、行号、状态、验证结果等；仅改链接或仅改文件名时可承接旧记录 |
| **files** | files.json | 已扫描文件索引：每个 Markdown 文件的路径、修改时间、内容哈希、包含的网络图片数量与 ID 列表、扫描状态等，用于增量扫描时判断是否需要重新扫描 |
| **metadata** | metadata.json | 系统元数据：如最后完整扫描时间、扫描文件总数、网络图片总数、缓存命中率等统计信息 |
| **blacklist** | blacklist.json | 失效 URL 黑名单：验证失败的图片 URL 及其失效原因（超时、404、403、网络错误等）、来源位置、检测时间等，用于避免重复验证失效链接 |
...
在插件设置 → 网络图片中可一键清空所有 IndexedDB 缓存数据：

- **清除范围**：images（图片记录）、files（文件索引）、metadata（元数据）、blacklist（黑名单）
- **实现方式**：调用 `FileCacheAdapter.clearAll()` 方法
- **效果**：下次扫描将重新建立所有缓存
- **适用场景**：
  - 缓存数据异常或损坏
  - 需要完全重新开始扫描
  - 释放存储空间
- **注意事项**：此操作不可恢复，清除后所有网络图片记录需要重新扫描
